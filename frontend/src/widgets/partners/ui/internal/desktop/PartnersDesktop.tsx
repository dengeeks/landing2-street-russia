      ...
      {data.map((cat, catIndex) => (
        cat.partners && cat.partners.length > 0 && (
          <div key={catIndex} className={styles.partnersGroup}>
            <h3 className={cat.partner_type.toLowerCase() === 'организаторы' ? styles.organizersTitle : ''}>
              {cat.partner_type.toLowerCase() === 'организаторы' ? cat.partner_type : `${cat.partner_type} партнеры`}
            </h3>
